...
class TAlistViewModel(application: Application): AndroidViewModel(application) {

    val tugasAkhirLD = MutableLiveData<List<TA>>()
    val loadingErrorLD = MutableLiveData<Boolean>()
    val loadingDoneLD = MutableLiveData<Boolean>()

    fun refresh(fakultas:String) {
        ...
        var taList:ArrayList<TA> =  arrayListOf<TA>()
        if (fakultas == "Teknik")
        {
            taList = arrayListOf<TA>(ta1)
        }
        else if (fakultas == "Psikologi")
        {
            taList = arrayListOf<TA>(ta2)
        }
        else
        {
            taList = arrayListOf<TA>(ta1, ta2)
        }
        tugasAkhirLD.value = taList
    }
